...
// Meses por extenso e abreviados, em portugues e ingles (o Drive pode exportar
// as datas ja escritas, conforme o formato da folha).
...
// Le uma data da folha e devolve YYYY-MM-DD (ou null se nao reconhecer).
// Aceita, entre outros: 10/05/2026, 10-05-2026, 10.05.2026, 2026-05-10,
// 10/05/26, "sab, 10/05/2026", "10 de maio de 2026", "10 mai 2026",
// "maio 10, 2026" e o numero de serie das folhas de calculo.
function parseDataFolha(bruto: string): string | null {
  let s = (bruto ?? '').trim();
  if (!s) return null;

  // Hora, se a folha a trouxer junto (22:00, 22h00, 22h). Nunca se inventa uma
  // hora: sem hora na folha, o concerto fica so com o dia.
  let hora = '';
  ...
  // Tira um dia da semana no inicio ("sabado,", "sab.", "sat"), se existir.
  // Lista explicita, para nao comer por engano o nome de um mes.
  s = s.replace(
  ...
  // DD/MM/AAAA, DD-MM-AA, DD.MM.AAAA (formato portugues: dia primeiro)
  m = s.match(/^(\d{1,2})[-\/.](\d{1,2})[-\/.](\d{2}|\d{4})$/);
  if (m) return comHora(`${noAno(m[3])}-${doisDigitos(m[2])}-${doisDigitos(m[1])}`);

  // 10 de maio de 2026 | 10 maio 2026 | 10 mai. 26 | 10-mai-2026
  ...
  // Numero de serie das folhas de calculo (dias desde 30/12/1899).
  m = s.match(/^(\d{5})$/);
  if (m) {
  ...
  // So o ano: fica sem data (nao inventa um dia 1 de Janeiro errado).
  return null;
}
...
export async function importarConcertos(texto: string): Promise<string> {
  ...
  // permite corrigir de uma vez uma importacao anterior sem datas.
  ...
  for (const linha of (texto ?? '').split('\n')) {
    ...
    // Concerto: 1) o mesmo nome e data ja existe, nao mexe;
    //           2) existe com o mesmo nome mas sem data, preenche-lhe a data;
    //           3) senao, cria.
