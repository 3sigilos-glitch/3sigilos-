...
// Envolve uma seccao reservada ao admin: risca violeta a esquerda e etiqueta.
export function SeccaoAdmin({ titulo, children }: { titulo: string; children: React.ReactNode }) {
  return (
    <div
      style={{
        display: 'flex',
        ...
        gap: 10,
        paddingLeft: 12,
        borderLeft: '3px solid var(--admin-linha)',
      }}
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        <h2 className="rotulo-seccao" style={{ color: 'var(--admin-forte)' }}>{titulo}</h2>
        <EtiquetaAdmin />
      </div>
      {children}
    </div>
